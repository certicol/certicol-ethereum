...
contract('CerticolDAO', function(accounts) {
    ...
    describe('Initialization and Deployment', function() {
        ...
        it('should initialize token-related mappings', async function() {
            expect(await contractInstance.getTokensLocked.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected 0 tokens locked as default
            expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected 0 voting rights as default
            expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected 0 PoSaT credits as default
            expect(await contractInstance.getLockedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected 0 locked PoSaT credits as default
        });

        it('should initialize delegate-related mappings', async function() {
            expect(await contractInstance.getDelegatedVotingRights.call(accounts[0], constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(0)); // Expected 0 delegated voting rights as default
            expect(await contractInstance.getNetDelegatedVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected 0 net delegated voting rights as default
            expect(await contractInstance.getDelegatedPoSaT.call(accounts[0], constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(0)); // Expected 0 delegated PoSaT credits as default
            expect(await contractInstance.getNetDelegatedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected 0 net delegated PoSaT credits as default
        });

        it('should initialize O10-related getter functions', async function() {
            expect(await contractInstance.getO10Requirements.call()).to.be.bignumber.equal(new BN("90000000" + "0".repeat(18)).div(new BN(10))); // Expected 10% of initial supply as O10 requirements
            expect(await contractInstance.getO10Status.call(accounts[1])).to.be.false; // Expected no O10 authorization by default
            expect(await contractInstance.getVOCRequirement.call()).to.be.bignumber.equal(new BN("10000" + "0".repeat(18))); // Expected 10,000 CDT required to vote confidence
            expect(await contractInstance.getActiveVoCIssued.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected 0 active vote by default
            expect(await contractInstance.getVoC.call(accounts[2])).to.have.lengthOf(0); // Expected array length of 0
            expect(await contractInstance.getVoCFrom.call(accounts[2], accounts[1])).to.be.false; // Expected no voc from accounts[1] to accounts[2] by default
            expect(await contractInstance.getCurrentPoSaTReward.call()).to.be.bignumber.equal(new BN(5)); // Expected 5% PoSaT reward by default
            expect(await contractInstance.getCurrentPoSaTRequirement.call()).to.be.bignumber.equal(new BN(2102400)); // Expected 2102400 blocks required per reward cycle by default
            expect(await contractInstance.getCurrentRingOneRequirement.call()).to.be.bignumber.equal(new BN(25)); // Expected 25% cumulative PoSaT credits required for ring 1 status
            expect(await contractInstance.getCurrentRing.call(accounts[2])).to.be.bignumber.equal(new BN(4)); // Expected ring 4 validation status by default
        });

        it('should initialize O5-related getter functions', async function() {
            expect(await contractInstance.getSeedUsed.call(new BN(0))).to.be.false; // Expected seed to be unused
            expect(await contractInstance.getDAODissolved.call()).to.be.false; // Expected DAO to be NOT dissolved
            expect(await contractInstance.getO5VoteNoConfidence.call(constants.ZERO_ADDRESS)).to.be.false; // Expected any account would not be voted no confidence by default
        });
    ...
    describe('Basic Token Lock and Release Mechanics', function() {
        ...
        it('should accept token deposit and lock those token', async function() {
            ...
            expect(await contractInstance.getTokensLocked.call(accounts[0])).to.be.bignumber.equal(new BN(100)); // Expected accounts[0] now has 100 locked tokens
            expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(100)); // Expected accounts[0] now has 100 voting rights
            expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(100)); // Expected accounts[0] now has 100 PoSaT credits
            expect(await contractInstance.getLockedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected accounts[0] has no locked PoSaT credits
            expect(await contractInstance.getCumulativeTokenLocked.call()).to.be.bignumber.equal(new BN(100)); // Expected 100 cumulative tokens locked
        });
        ...
        it('should revert token deposit if deposit more than what they owned', async function() {
            ...
            expect(await contractInstance.getTokensLocked.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected accounts[1] has 0 locked tokens
            expect(await contractInstance.getVotingRights.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected accounts[1] has 0 voting rights
            expect(await contractInstance.getAvailablePoSaT.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected accounts[1] has 0 PoSaT credits
            expect(await contractInstance.getLockedPoSaT.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected accounts[1] has no locked PoSaT credits
            expect(await contractInstance.getCumulativeTokenLocked.call()).to.be.bignumber.equal(new BN(0)); // Expected 0 cumulative tokens locked
        });
        ...
        it('should revert token deposit if it is not the designated CDT token', async function() {
            ...
            expect(await contractInstance.getVotingRights.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected accounts[1] has 0 voting rights
            expect(await contractInstance.getAvailablePoSaT.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected accounts[1] has 0 PoSaT credits
            expect(await contractInstance.getLockedPoSaT.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected accounts[1] has no locked PoSaT credit
            expect(await contractInstance.getCumulativeTokenLocked.call()).to.be.bignumber.equal(new BN(0)); // Expected 0 cumulative tokens locked
        });
        ...
        it('should accept withdrawl of locked tokens and adjust voting rights and PoSaT credit accordingly', async function() {
            ...
            expect(await contractInstance.getTokensLocked.call(accounts[0])).to.be.bignumber.equal(new BN(100).sub(new BN(50))); // Expected accounts[0] now has 100 - 50 locked tokens only
            expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(100).sub(new BN(50))); // Expected accounts[0] now has 100 - 50 voting rights only
            expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(100).sub(new BN(50))); // Expected accounts[0] now has 100 - 50 PoSaT credits only
            expect(await contractInstance.getLockedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected accounts[0] has no locked PoSaT credits
            expect(await contractInstance.getCumulativeTokenLocked.call()).to.be.bignumber.equal(new BN(50)); // Expected 50 cumulative tokens locked
        });
        ...
        it('should not accept withdrawl of locked tokens if it exceeds the number of tokens they have locked', async function() {
            ...
            expect(await contractInstance.getTokensLocked.call(accounts[0])).to.be.bignumber.equal(new BN(100)); // Expected accounts[0] still has 100 locked tokens
            expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(100)); // Expected accounts[0] still has 100 voting rights
            expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(100)); // Expected accounts[0] still has 100 PoSaT credits
            expect(await contractInstance.getLockedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected accounts[0] has no locked PoSaT credits
            expect(await contractInstance.getCumulativeTokenLocked.call()).to.be.bignumber.equal(new BN(100)); // Expected 100 cumulative tokens locked
        });
    ...
    describe('Delegation Mechanics', function() {
        ...
        describe('Voting Rights Delegation Mechanics', function() {
            ...
            it('should accept delegation of voting rights', async function() {
                ...
                expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected 100 - 100 voting rights left in accounts[0]
                expect(await contractInstance.getVotingRights.call(constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(100)); // Expected 100 voting rights (delegated from accounts[0]) in 0x0
                expect(await contractInstance.getDelegatedVotingRights.call(accounts[0], constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(100)); // Expected 100 delegated rights from accounts[0] to 0x0
                expect(await contractInstance.getNetDelegatedVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(100)); // Expected a net of 100 delegated rights from accounts[0]
            });
            ...
            it('should accept increased delegation of voting rights', async function() {
                ...
                expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected 100 - 50 - 50 voting rights left in accounts[0]
                expect(await contractInstance.getVotingRights.call(constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(100)); // Expected 100 voting rights (delegated from accounts[0]) in 0x0
                expect(await contractInstance.getDelegatedVotingRights.call(accounts[0], constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(100)); // Expected 100 delegated rights from accounts[0] to 0x0
                expect(await contractInstance.getNetDelegatedVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(100)); // Expected a net of 100 delegated rights from accounts[0]
            });        
            ...
            it('should not accept delegation if it exceeds the amount of voting rights msg.sender owns', async function() {
                ...
                expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected 100 - 50 voting rights left in accounts[0]
                expect(await contractInstance.getVotingRights.call(constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(50)); // Expected 50 voting rights (delegated from accounts[0]) in 0x0
                expect(await contractInstance.getVotingRights.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected 0 voting rights in accounts[1]
                expect(await contractInstance.getDelegatedVotingRights.call(accounts[0], constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(50)); // Expected 50 delegated rights from accounts[0] to 0x0
                expect(await contractInstance.getDelegatedVotingRights.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected 0 delegated rights from accounts[0] to accounts[1]
                expect(await contractInstance.getNetDelegatedVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected a net of 50 delegated rights from accounts[0]
            });
            ...
            it('should not accept secondary delegation', async function() {
                ...
                expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected 100 - 50 voting rights left in accounts[0]
                expect(await contractInstance.getVotingRights.call(accounts[1])).to.be.bignumber.equal(new BN(50)); // Expected 50 voting rights (delegated from accounts[0]) in accounts[1]
                expect(await contractInstance.getVotingRights.call(accounts[2])).to.be.bignumber.equal(new BN(0)); // Expected 0 voting rights in accounts[2]
                expect(await contractInstance.getDelegatedVotingRights.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(50)); // Expected 50 delegated rights from accounts[0] to accounts[1]
                expect(await contractInstance.getDelegatedVotingRights.call(accounts[1], accounts[2])).to.be.bignumber.equal(new BN(0)); // Expected 0 delegated rights from accounts[1] to accounts[2]
                expect(await contractInstance.getNetDelegatedVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected a net of 50 delegated rights from accounts[0]
                expect(await contractInstance.getNetDelegatedVotingRights.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected a net of 0 delegated rights from accounts[1]
            });
            ...
            it('should accept the withdrawl of locked tokens if msg.sender still have sufficient voting rights', async function() {
                ...
                expect(await contractInstance.getTokensLocked.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected accounts[0] has 100 - 50 locked tokens only
                expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected 100 - 50 - 50 voting rights left in accounts[0]
                expect(await contractInstance.getVotingRights.call(accounts[1])).to.be.bignumber.equal(new BN(50)); // Expected 50 voting rights (delegated from accounts[0]) in accounts[1]
                expect(await contractInstance.getDelegatedVotingRights.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(50)); // Expected 50 delegated rights from accounts[0] to accounts[1]
                expect(await contractInstance.getNetDelegatedVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected a net of 50 delegated rights from accounts[0]
            ...
            it('should accept complete withdrawl of delegated voting rights', async function() {
                ...
                expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(100)); // Expected 100 - 50 + 50 voting rights left in accounts[0]
                expect(await contractInstance.getVotingRights.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected 50 - 50 voting rights in accounts[1]
                expect(await contractInstance.getDelegatedVotingRights.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected 0 delegated rights from accounts[0] to accounts[1]
                expect(await contractInstance.getNetDelegatedVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected a net of 0 delegated rights from accounts[0]
            });
            ...
            it('should accept decreased in delegated voting rights', async function() {
                ...
                expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(90)); // Expected 100 - 50 + 40 voting rights left in accounts[0]
                expect(await contractInstance.getVotingRights.call(accounts[1])).to.be.bignumber.equal(new BN(10)); // Expected 50 - 40 voting rights in accounts[1]
                expect(await contractInstance.getDelegatedVotingRights.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(10)); // Expected 10 delegated rights from accounts[0] to accounts[1]
                expect(await contractInstance.getNetDelegatedVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(10)); // Expected a net of 10 delegated rights from accounts[0]
            });
            ...
            it('should not accept withdrawl of delegated voting rights if amount > total delegated voting rights', async function() {
                ...
                expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected 100 - 50 voting rights still left in accounts[0]
                expect(await contractInstance.getVotingRights.call(accounts[1])).to.be.bignumber.equal(new BN(50)); // Expected 50 voting rights still in accounts[1]
                expect(await contractInstance.getDelegatedVotingRights.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(50)); // Expected 50 delegated rights from accounts[0] to accounts[1]
                expect(await contractInstance.getNetDelegatedVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected a net of 0 delegated rights from accounts[0]
            });
            ...
            it('should not accept withdrawl of delegated voting rights if amount > delegated voting rights to the delegate', async function() {
                ...
                expect(await contractInstance.getVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(20)); // Expected 100 - 80 voting rights still left in accounts[0]
                expect(await contractInstance.getVotingRights.call(accounts[1])).to.be.bignumber.equal(new BN(40)); // Expected 40 voting rights still in accounts[1]
                expect(await contractInstance.getVotingRights.call(accounts[2])).to.be.bignumber.equal(new BN(40)); // Expected 40 voting rights still in accounts[2]
                expect(await contractInstance.getDelegatedVotingRights.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(40)); // Expected 40 delegated rights from accounts[0] to accounts[1]
                expect(await contractInstance.getNetDelegatedVotingRights.call(accounts[0])).to.be.bignumber.equal(new BN(80)); // Expected a net of 80 delegated rights from accounts[0]
            });
        ...
        describe('PoSaT Credits Delegation Mechanics', function() {
            ...
            it('should accept delegation of PoSaT credits', async function() {
                ...
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected 100 - 100 PoSaT credits left in accounts[0]
                expect(await contractInstance.getAvailablePoSaT.call(constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(100)); // Expected 100 PoSaT credits (delegated from accounts[0]) in 0x0
                expect(await contractInstance.getDelegatedPoSaT.call(accounts[0], constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(100)); // Expected 100 delegated PoSaT credits from accounts[0] to 0x0
                expect(await contractInstance.getNetDelegatedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(100)); // Expected a net of 100 delegated PoSaT credits from accounts[0]
            });
            ...
            it('should accept increased delegation of PoSaT credits', async function() {
                ...
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected 100 - 50 - 50 PoSaT credits left in accounts[0]
                expect(await contractInstance.getAvailablePoSaT.call(constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(100)); // Expected 100 PoSaT credits (delegated from accounts[0]) in 0x0
                expect(await contractInstance.getDelegatedPoSaT.call(accounts[0], constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(100)); // Expected 100 delegated PoSaT credits from accounts[0] to 0x0
                expect(await contractInstance.getNetDelegatedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(100)); // Expected a net of 100 delegated PoSaT credits from accounts[0]
            });        
            ...
            it('should not accept delegation if it exceeds the amount of PoSaT credits msg.sender owns', async function() {
                ...
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected 100 - 50 PoSaT credits left in accounts[0]
                expect(await contractInstance.getAvailablePoSaT.call(constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(50)); // Expected 50 PoSaT credits (delegated from accounts[0]) in 0x0
                expect(await contractInstance.getAvailablePoSaT.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected 0 PoSaT credits in accounts[1]
                expect(await contractInstance.getDelegatedPoSaT.call(accounts[0], constants.ZERO_ADDRESS)).to.be.bignumber.equal(new BN(50)); // Expected 50 delegated PoSaT credits from accounts[0] to 0x0
                expect(await contractInstance.getDelegatedPoSaT.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected 0 delegated PoSaT credits from accounts[0] to accounts[1]
                expect(await contractInstance.getNetDelegatedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected a net of 50 delegated PoSaT credits from accounts[0]
            });
            ...
            it('should not accept secondary delegation', async function() {
                ...
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected 100 - 50 PoSaT credits left in accounts[0]
                expect(await contractInstance.getAvailablePoSaT.call(accounts[1])).to.be.bignumber.equal(new BN(50)); // Expected 50 PoSaT credits (delegated from accounts[0]) in accounts[1]
                expect(await contractInstance.getAvailablePoSaT.call(accounts[2])).to.be.bignumber.equal(new BN(0)); // Expected 0 PoSaT credits in accounts[2]
                expect(await contractInstance.getDelegatedPoSaT.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(50)); // Expected 50 delegated PoSaT credits from accounts[0] to accounts[1]
                expect(await contractInstance.getDelegatedPoSaT.call(accounts[1], accounts[2])).to.be.bignumber.equal(new BN(0)); // Expected 0 delegated PoSaT credits from accounts[1] to accounts[2]
                expect(await contractInstance.getNetDelegatedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected a net of 50 delegated PoSaT credits from accounts[0]
                expect(await contractInstance.getNetDelegatedPoSaT.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected a net of 0 delegated PoSaT credits from accounts[1]
            });
            ...
            it('should accept complete withdrawl of delegated PoSaT credits', async function() {
                ...
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(100)); // Expected 100 - 50 + 50 PoSaT credits left in accounts[0]
                expect(await contractInstance.getAvailablePoSaT.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected 50 - 50 PoSaT credits in accounts[1]
                expect(await contractInstance.getDelegatedPoSaT.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected 0 delegated PoSaT credits from accounts[0] to accounts[1]
                expect(await contractInstance.getNetDelegatedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected a net of 0 delegated PoSaT credits from accounts[0]
            });
            ...
            it('should accept decreased in delegated PoSaT credits', async function() {
                ...
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(90)); // Expected 100 - 50 + 40 PoSaT credits left in accounts[0]
                expect(await contractInstance.getAvailablePoSaT.call(accounts[1])).to.be.bignumber.equal(new BN(10)); // Expected 50 - 40 PoSaT credits in accounts[1]
                expect(await contractInstance.getDelegatedPoSaT.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(10)); // Expected 10 delegated PoSaT credits from accounts[0] to accounts[1]
                expect(await contractInstance.getNetDelegatedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(10)); // Expected a net of 10 delegated PoSaT credits from accounts[0]
            });
            ...
            it('should not accept withdrawl of delegated PoSaT credits if amount > total delegated PoSaT credits', async function() {
                ...
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected 100 - 50 PoSaT credits still left in accounts[0]
                expect(await contractInstance.getAvailablePoSaT.call(accounts[1])).to.be.bignumber.equal(new BN(50)); // Expected 50 PoSaT credits still in accounts[1]
                expect(await contractInstance.getDelegatedPoSaT.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(50)); // Expected 50 delegated PoSaT credits from accounts[0] to accounts[1]
                expect(await contractInstance.getNetDelegatedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(50)); // Expected a net of 0 delegated PoSaT credits from accounts[0]
            });
            ...
            it('should not accept withdrawl of delegated PoSaT credits if amount > delegated PoSaT credits to the delegate', async function() {
                ...
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(20)); // Expected 100 - 80 PoSaT credits still left in accounts[0]
                expect(await contractInstance.getAvailablePoSaT.call(accounts[1])).to.be.bignumber.equal(new BN(40)); // Expected 40 PoSaT credits still in accounts[1]
                expect(await contractInstance.getAvailablePoSaT.call(accounts[2])).to.be.bignumber.equal(new BN(40)); // Expected 40 PoSaT credits still in accounts[2]
                expect(await contractInstance.getDelegatedPoSaT.call(accounts[0], accounts[1])).to.be.bignumber.equal(new BN(40)); // Expected 40 delegated PoSaT credits from accounts[0] to accounts[1]
                expect(await contractInstance.getNetDelegatedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(80)); // Expected a net of 80 delegated PoSaT credits from accounts[0]
            });
    ...
    describe('PoSaT Mechanics', function() {
        ...
        describe('O10 Authorization and Deauthorization', function() {
            ...
            it('should grant 010 authorization using non-delegated credits', async function() {
                ...
                let O10Requirement = await contractInstance.getO10Requirements.call(); // Get O10 requirement
                expectEvent.inLogs(tx.logs, 'O10Authorized', { O10: accounts[0], PoSaTLocked: O10Requirement }); // Expect O10Authorized event
                expect(await contractInstance.getO10Status.call(accounts[0])).to.be.true; // Expect O10 status granted
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(TOKEN_LOCKED.sub(O10Requirement)); // Expected available PoSaT to drop by O10 requirement
                expect(await contractInstance.getLockedPoSaT.call(accounts[0])).to.be.bignumber.equal(O10Requirement); // Expected O10 requirement to be locked
            });
            ...
            it('should grant 010 authorization using delegated credits', async function() {
                ...
                expect(await contractInstance.getO10Status.call(accounts[1])).to.be.true; // Expect O10 status granted
                expect(await contractInstance.getAvailablePoSaT.call(accounts[1])).to.be.bignumber.equal(new BN(0)); // Expected available PoSaT to drop by O10 requirement - 0
                expect(await contractInstance.getLockedPoSaT.call(accounts[1])).to.be.bignumber.equal(O10Requirement); // Expected O10 requirement to be locked
            });
            ...
            it('should allow deauthorization of O10 status', async function() {
                ...
                expect(await contractInstance.getO10Status.call(accounts[0])).to.be.false; // Expect O10 status removed
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(TOKEN_LOCKED); // Expected available PoSaT to be identical to token locked
                expect(await contractInstance.getLockedPoSaT.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expected 0 credits to be locked
            });

            it('should revert deauthorization if there are existing active vote of confidence', async function() {
                let O10Requirement = await contractInstance.getO10Requirements.call(); // Get O10 requirement
                let vocRequirement = await contractInstance.getVOCRequirement.call(); // Get vote of confidence requirement
                await contractInstance.O10Authorization({ from: accounts[0] }); // Obtain O10 authorization which should succeed
                await contractInstance.O10VoteConfidence(accounts[1], { from: accounts[0] }); // Vote confidence on accounts[1]
                expect(await contractInstance.getActiveVoCIssued.call(accounts[0])).to.be.bignumber.equal(new BN(1)); // Expect 1 active vote of confidence now
                ...
                expect(await contractInstance.getO10Status.call(accounts[0])).to.be.true; // Expect O10 status still persist
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(TOKEN_LOCKED.sub(O10Requirement).sub(vocRequirement)); // Expected available PoSaT to decrease by O10 requirement and VoC requirement
                expect(await contractInstance.getLockedPoSaT.call(accounts[0])).to.be.bignumber.equal(O10Requirement.add(vocRequirement)); // Expected O10 requirement and 1 vote of confidence credits to remain locked
            });
        ...
        describe('O10 Vote of Confidence', function() {
            ...
            it('should allow vote of confidence', async function() {
                ...
                expect(await contractInstance.getVoC.call(accounts[1])).to.include.members([ accounts[0] ]); // Expect accounts[0] in reverse VoC record
                expect(await contractInstance.getVoCFrom.call(accounts[1], accounts[0])).to.be.true; // Expect accounts[0] to have endorsed accounts[1] in VoC record
                expect(await contractInstance.getActiveVoCIssued.call(accounts[0])).to.be.bignumber.equal(new BN(1)); // Expect 1 active vote of confidence at the moment
                expect(await contractInstance.getLockedPoSaT.call(accounts[0])).to.be.bignumber.equal(O10Requirement.add(vocRequirement)); // Expect O10 requirement and VoC requirement to be locked
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(TOKEN_LOCKED.sub(O10Requirement).sub(vocRequirement)); // Expect available credits to drop by O10 requirement and VoC requirement
            });
            ...
            it('should allow further vote of confidence', async function() {
                ...
                expect(await contractInstance.getVoC.call(accounts[1])).to.include.members([ accounts[0], accounts[2] ]); // Expect accounts[0] and accounts[2] in reverse VoC record
                expect(await contractInstance.getVoCFrom.call(accounts[1], accounts[0])).to.be.true; // Expect accounts[0] to have endorsed accounts[1] in VoC record
                expect(await contractInstance.getVoCFrom.call(accounts[1], accounts[2])).to.be.true; // Expect accounts[2] to have endorsed accounts[1] in VoC record
                expect(await contractInstance.getActiveVoCIssued.call(accounts[0])).to.be.bignumber.equal(new BN(1)); // Expect 1 active vote from accounts[0] of confidence at the moment
                expect(await contractInstance.getActiveVoCIssued.call(accounts[2])).to.be.bignumber.equal(new BN(1)); // Expect 1 active vote from accounts[2] of confidence at the moment
                expect(await contractInstance.getLockedPoSaT.call(accounts[0])).to.be.bignumber.equal(O10Requirement.add(vocRequirement)); // Expect O10 requirement and VoC requirement to be locked in accounts[0] (P.S. delegation would NOT increase locked PoSaT credits, as it is only affected by participation in the PoSaT mechanism)
                expect(await contractInstance.getLockedPoSaT.call(accounts[2])).to.be.bignumber.equal(O10Requirement.add(vocRequirement)); // Expect O10 requirement and VoC requirement to be locked in accounts[2]
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(TOKEN_LOCKED.sub(O10Requirement).sub(vocRequirement).sub(O10Requirement).sub(vocRequirement)); // Expect available credits to drop by 2x O10 requirement and 2x VoC requirement in accounts[0]
                expect(await contractInstance.getAvailablePoSaT.call(accounts[2])).to.be.bignumber.equal(new BN(0)); // Expect available credits be 0 in accounts[2]
            });
            ...
            it('should allow revoke of vote of confidence', async function() {
                ...
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(TOKEN_LOCKED.sub(O10Requirement)); // Expect available credits to drop by O10 requirement only (VoC requirement unlocked)
                expect(await contractInstance.getLockedPoSaT.call(accounts[0])).to.be.bignumber.equal(O10Requirement); // Expect only O10 requirement to be locked (VoC requirement unlocked)
                expect(await contractInstance.getActiveVoCIssued.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expect 0 active vote of confidence
                expect(await contractInstance.getVoCFrom.call(accounts[1], accounts[0])).to.be.false; // Expect accounts[0] to no longer endorse accounts[1] in VoC record
                expect(await contractInstance.getVoC.call(accounts[1])).to.include.members([ constants.ZERO_ADDRESS ]); // Expect 1 zero address in reverse VoC record for accounts[1]
            });
            ...
            it('should allow 2 vote of confidence and revoke the first vote of confidence', async function() {
                ...
                expectEvent.inLogs(tx.logs, 'O10RevokeVoteConfidence', { O10: accounts[0], target: accounts[1], PoSaTUnlocked: vocRequirement }); // Expect O10RevokeVoteConfidence event
                expect(await contractInstance.getAvailablePoSaT.call(accounts[0])).to.be.bignumber.equal(TOKEN_LOCKED.sub(O10Requirement).sub(O10Requirement).sub(vocRequirement)); // Expect available credits to drop by O10 requirement only (VoC requirement unlocked), and another O10 requirement and VoC requirement locked due to delegation, in accounts[0]
                expect(await contractInstance.getAvailablePoSaT.call(accounts[2])).to.be.bignumber.equal(new BN(0)); // Expect 0 available credits in accounts[2]
                expect(await contractInstance.getLockedPoSaT.call(accounts[0])).to.be.bignumber.equal(O10Requirement); // Expect only O10 requirement to be locked (VoC requirement unlocked) in accounts[0]
                expect(await contractInstance.getLockedPoSaT.call(accounts[2])).to.be.bignumber.equal(O10Requirement.add(vocRequirement)); // Expect O10 requirement and VoC requirement to be locked in accounts[2]
                expect(await contractInstance.getActiveVoCIssued.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expect 0 active vote of confidence in accounts[0]
                expect(await contractInstance.getActiveVoCIssued.call(accounts[2])).to.be.bignumber.equal(new BN(1)); // Expect 1 active vote of confidence in accounts[2]
                expect(await contractInstance.getVoCFrom.call(accounts[1], accounts[0])).to.be.false; // Expect accounts[0] to no longer endorse accounts[1] in VoC record
                expect(await contractInstance.getVoCFrom.call(accounts[1], accounts[2])).to.be.true; // Expect accounts[2] to continue to endorse accounts[1] in VoC record
                expect(await contractInstance.getVoC.call(accounts[1])).to.include.members([ constants.ZERO_ADDRESS, accounts[2] ]); // Expect accounts[2] and address(0) in reverse VoC record for accounts[1]
            });
            ...
            it('should allow 2 vote of confidence and revoke the second vote of confidence', async function() {
                ...
                expect(await contractInstance.getAvailablePoSaT.call(accounts[2])).to.be.bignumber.equal(vocRequirement); // Expect vocRequirement available credits in accounts[2] since it is now unlocked
                expect(await contractInstance.getLockedPoSaT.call(accounts[0])).to.be.bignumber.equal(O10Requirement.add(vocRequirement)); // Expect O10 requirement and VoC requirement to be locked in accounts[0]
                expect(await contractInstance.getLockedPoSaT.call(accounts[2])).to.be.bignumber.equal(O10Requirement); // Expect only O10 requirement to be locked (VoC requirement unlocked) in accounts[2]
                expect(await contractInstance.getActiveVoCIssued.call(accounts[0])).to.be.bignumber.equal(new BN(1)); // Expect 1 active vote of confidence in accounts[0]
                expect(await contractInstance.getActiveVoCIssued.call(accounts[2])).to.be.bignumber.equal(new BN(0)); // Expect 0 active vote of confidence in accounts[2]
                expect(await contractInstance.getVoCFrom.call(accounts[1], accounts[0])).to.be.true; // Expect accounts[0] to continue to endorse endorse accounts[1] in VoC record
                expect(await contractInstance.getVoCFrom.call(accounts[1], accounts[2])).to.be.false; // Expect accounts[2] to no longer endorse accounts[1] in VoC record
                expect(await contractInstance.getVoC.call(accounts[1])).to.include.members([ accounts[0], constants.ZERO_ADDRESS ]); // Expect accounts[0] and address(0) in reverse VoC record for accounts[1]
            });
        ...
        describe('PoSaT Reward', function() {
            ...
            it('should give out a single PoSaT rewards', async function() {
                ...
                let vocRequirement = await contractInstance.getVOCRequirement.call(); // Get vote of confidence requirement
                let blockRequirement = await contractInstance.getCurrentPoSaTRequirement.call(); // Get current PoSaT block requirement
                let interestRate = await contractInstance.getCurrentPoSaTReward.call(); // Get current PoSaT interest rate
        ...
        describe('Ring I validation', function() {
            ...
            it('should default to ring 2 status without PoSaT vote', async function() {
                ...
                expect(await contractInstance.getCurrentRing.call(addressValid)).to.be.bignumber.equal(new BN(2)); // Expect ring 2 after CerticolCA validation
            });
            ...
            it('should default to ring 2 status with PoSaT vote but cumulative credit held did not qualify for ring 1 status', async function() {
                ...
                expect(await contractInstance.getCurrentRing.call(addressValid)).to.be.bignumber.equal(new BN(2)); // Expect ring 2 after 25% - 1 credits endorsement
            });
            ...
            it('should grant ring 1 status when cumulative credit held qualify for ring 1 status', async function() {
                ...
                expect(await contractInstance.getCurrentRing.call(addressValid)).to.be.bignumber.equal(new BN(1)); // Expect ring 1 after 50% - 1 credits endorsement
            });
            ...
            it('should process revoke vote correctly', async function() {
                ...
                expect(await contractInstance.getCurrentRing.call(addressValid)).to.be.bignumber.equal(new BN(2)); // Expect ring 2 after only 25% - 1 credits endorsement
                ...
                expect(await contractInstance.getCurrentRing.call(addressValid)).to.be.bignumber.equal(new BN(1)); // Expect ring 1 after 25% credits endorsement
            });
    ...
    describe('O5 Mechanics', function() {
        ...
        describe('O5 Authorization mechanics', function() {
            ...
            it('should allow O5 authorization', async function() {
                ...
                expect(await contractInstance.getSeedUsed.call(oneTimeSeed)).to.be.true; // Expect one time seed to be used now
            });
            ...
            it('should revert O5 authorization if cumulative voting rights did not exceeds 50% of total voting rights', async function() {
                ...
                expect(await contractInstance.getSeedUsed.call(oneTimeSeed)).to.be.false; // Expect one time seed to still be not used now
            });
        ...
        describe('O5 Modify Commands', function() {
            ...
            it('should allow O5 to modify ring one requirement', async function() {
                ...
                expect(await contractInstance.getCurrentRingOneRequirement.call()).to.be.bignumber.equal(new BN(newRingOneRequirement)); // Expect ring one requirement to be modified to newRingOneRequirement
            });
            ...
            it('should revert if non-O5 try to modify ring one requirement', async function() {
                ...
                expect(await contractInstance.getCurrentRingOneRequirement.call()).to.be.bignumber.not.equal(new BN(newRingOneRequirement)); // Expect ring one requirement to be unchanged
            });
            ...
            it('should allow O5 to modify PoSaT requirement', async function() {
                ...
                expect(await contractInstance.getCurrentPoSaTRequirement.call()).to.be.bignumber.equal(new BN(newPoSaTRequirement)); // Expect PoSaT requirement to be modified to newPoSaTRequirement
            });
            ...
            it('should revert if non-O5 try to modify PoSaT requirement', async function() {
                ...
                expect(await contractInstance.getCurrentPoSaTRequirement.call()).to.be.bignumber.not.equal(new BN(newPoSaTRequirement)); // Expect PoSaT requirement to be unchanged
            });
            ...
            it('should allow O5 to modify PoSaT reward', async function() {
                ...
                expect(await contractInstance.getCurrentPoSaTReward.call()).to.be.bignumber.equal(new BN(newPoSaTReward)); // Expect PoSaT reward to be modified to newPoSaTReward
            });
            ...
            it('should revert if non-O5 try to modify PoSaT reward', async function() {
                ...
                expect(await contractInstance.getCurrentPoSaTReward.call()).to.be.bignumber.not.equal(new BN(newPoSaTReward)); // Expect PoSaT reward to be unchanged
            });
            ...
            it('should allow O5 to modify vote of confidence requirement', async function() {
                ...
                expect(await contractInstance.getVOCRequirement.call()).to.be.bignumber.equal(new BN(newVoCReward)); // Expect vote of confidence requirement to be modified to newVoCReward
            });
            ...
            it('should revert if non-O5 try to modify vote of confidence requirement', async function() {
                ...
                expect(await contractInstance.getVOCRequirement.call()).to.be.bignumber.not.equal(new BN(newVoCReward)); // Expect vote of confidence requirement to be unchanged
            });
            ...
            it('should allow O5 to modify O10 requirement', async function() {
                ...
                expect(await contractInstance.getO10Requirements.call()).to.be.bignumber.equal(new BN(newO10Reward)); // Expect O10 requirement to be modified to newO10Reward
            });
            ...
            it('should revert if non-O5 try to modify O10 requirement', async function() {
                ...
                expect(await contractInstance.getO10Requirements.call()).to.be.bignumber.not.equal(new BN(newO10Reward)); // Expect O10 requirement to be unchanged
            });
        ...
        describe('O5 Dissolve Command', function() {
            ...
            it('should allow O5 to dissolve DAO', async function() {
                ...
                expect(await contractInstance.getDAODissolved.call()).to.be.true; // Expect DAO to be dissolved after this operation
            });
            ...
            it('should revert if non-O5 try dissolve DAO', async function() {
                ...
                expect(await contractInstance.getDAODissolved.call()).to.be.false; // Expect DAO to NOT be dissolved after this operation
            });
            ...
            it('should allow dissolveWithdrawl call when DAO is dissolved', async function() {
                ...
                expect(await contractInstance.getCumulativeTokenLocked.call()).to.be.bignumber.equal(new BN(0)); // Expected a total of 0 token locked in the contract now
                expect(await contractInstance.getTokensLocked.call(accounts[0])).to.be.bignumber.equal(new BN(0)); // Expect 0 token locked in contract from accounts[0] now
            });
            ...
            it('should revert dissolveWithdrawl call when DAO is not dissolved', async function() {
                ...
                expect(await contractInstance.getCumulativeTokenLocked.call()).to.be.bignumber.equal(INITIAL_SUPPLY.mul(new BN(60)).div(new BN(100))); // Expected a total of 60% token still locked in the contract
                expect(await contractInstance.getTokensLocked.call(accounts[0])).to.be.bignumber.equal(INITIAL_SUPPLY.mul(new BN(60)).div(new BN(100))); // Expect 60% token still locked in contract from accounts[0]
            });
        ...
        describe('O5 Vote of No Confidence', function() {
            ...
            it('should allow O5 to issue vote of no confidence', async function() {
                ...
                expect(await contractInstance.getO5VoteNoConfidence.call(addressValid)).to.be.true; // Expect addressValid to be voted no confidence
            });
            ...
            it('should revert if non-O5 try to issue vote of no confidence', async function() {
                ...
                expect(await contractInstance.getO5VoteNoConfidence.call(addressValid)).to.be.false; // Expect addressValid to NOT be voted no confidence
            });
            ...
            it('should return ring 4 status regardless of prior validation status once target was voted no confidence by O5', async function() {
                expect(await contractInstance.getCurrentRing.call(addressValid)).to.be.bignumber.equal(new BN(1)); // Expect ring 1 status originally
                await voteNoConfidence(contractInstance); // Vote no confidence using accounts[0]
                expect(await contractInstance.getCurrentRing.call(addressValid)).to.be.bignumber.equal(new BN(4)); // Expect ring 4 status afterward
            });
