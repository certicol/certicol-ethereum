...
// Test for CerticolDAODeployer.sol
contract('CerticolDAODeployer', function(accounts) {

    // Gas cost used to deploy contract, additional gas will be refunded
    var BLOCK_GAS_LIMIT = 8000000;

    // Deployed instance of GST1 contract
    var GasTokenContract;
    // Address of deployed GST1 contract
    var GasTokenAddress;
    // Address of deployed CerticolDAOToken contract
    var TokenAddress;
    // Address of deployed CerticolCA contract
    var CAAddress;
    ...
    before(async function() {
        ...
        // Deploy CerticolCA and CerticolDAOToken
        caInstance = await CerticolCA.new({ from: accounts[1] });
        tokenInstance = await CerticolDAOToken.new(accounts[0], { from: accounts[1] });
        // Record the deployed contract address
        CAAddress = caInstance.address;
    ...
    it('should deploy CerticolDAO.sol with fewer gas', async function() {
        ...
        let approveGasCost = await GasTokenContract.methods.approve(deployAddress, 243).estimateGas({ from: accounts[0] });
        // Approve the predicted contract address to free up to 243 GST1
        await GasTokenContract.methods.approve(deployAddress, 243).send({ from: accounts[0], gas: approveGasCost });
        // Deploy using CerticolDAODeployer and obtain the gas used
        let contract = await CerticolDAODeployer.new(GasTokenAddress, TokenAddress, CAAddress, { from: accounts[0], gas: BLOCK_GAS_LIMIT });
        let receipt = await web3.eth.getTransactionReceipt(contract.transactionHash);
        let gasUsed = receipt.cumulativeGasUsed;
        // Estimate the gas cost normally required to deploy CerticolDAO directly
        let gasCostExpected = await new web3.eth.Contract(CerticolDAO._json.abi)
            .deploy({ data: CerticolDAO._json.bytecode, arguments: [ TokenAddress, CAAddress ] })
            .estimateGas();
        // Assert fewer gas was used
        expect(gasUsed).below(gasCostExpected);
        // Predict the address of the deployed CerticolDAO contract
        ...
        expect(finalAddressBytecode).to.have.string(CerticolDAO._json.deployedBytecode);
    });

    it('should revert if insufficient GST token is approved to the contract', async function() {
        await expectRevert(
            CerticolDAODeployer.new(GasTokenAddress, TokenAddress, CAAddress, { from: accounts[0], gas: BLOCK_GAS_LIMIT }),
